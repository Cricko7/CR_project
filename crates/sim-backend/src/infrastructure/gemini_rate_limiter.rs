...
pub fn shared_gemini_rate_limiter(min_interval: Duration) -> Arc<GeminiRateLimiter> {
    static SHARED_LIMITER: OnceLock<Arc<GeminiRateLimiter>> = OnceLock::new();

    let limiter = SHARED_LIMITER
        .get_or_init(|| Arc::new(GeminiRateLimiter::new(min_interval)))
        .clone();
    ...
    if limiter.min_interval() != min_interval {
        tracing::warn!(
            ...
            "gemini rate limiter already initialized with another interval; active value is kept"
        );
    }
