...
#[derive(Debug)]
pub struct OpenRouterRateLimiter {
    min_interval: Duration,
    next_allowed_at: Mutex<Instant>,
}

impl OpenRouterRateLimiter {
...
pub fn shared_openrouter_rate_limiter(min_interval: Duration) -> Arc<OpenRouterRateLimiter> {
    static SHARED_LIMITER: OnceLock<Arc<OpenRouterRateLimiter>> = OnceLock::new();

    let limiter = SHARED_LIMITER
        .get_or_init(|| Arc::new(OpenRouterRateLimiter::new(min_interval)))
        .clone();
    ...
    if limiter.min_interval() != min_interval {
        tracing::warn!(
            ...
            "openrouter rate limiter already initialized with another interval; active value is kept"
        );
    }
